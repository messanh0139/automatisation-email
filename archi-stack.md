...
- **Incident du 6 juillet 2026 :** la relève IMAP (F1 révisé), filtrée uniquement sur le statut « non lu », a traité par erreur des milliers d'emails déjà présents dans la boîte réelle (historique), créant 9 tickets HubSpot indésirables avant d'être stoppée (mot de passe d'application révoqué en urgence). Aucun email n'a été réellement envoyé — protégé par le choix F4 (envoi sur déclenchement explicite, jamais automatique). Correctif : `IMAP_SINCE_DATE` exclut désormais tout email antérieur à la mise en service, quel que soit son statut lu/non lu ; un plafond de 10 emails par exécution limite aussi le risque en cas de nouvelle anomalie.
